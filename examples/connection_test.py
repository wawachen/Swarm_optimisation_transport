...
class Drone_Env(py_environment.PyEnvironment):
    ...
    def __init__(self,env_name):
        ...
        [m,self.m1]= self.pr.import_model(path.join(robot_DIR, 'Quadricopter_rope.ttm'))
    ...
    def _get_state(self):
        ...
        vector_state = np.r_[self.target.get_position()[:], self.target.get_orientation()[:],self.agent.get_drone_position()[:], self.agent.get_orientation()[:],dis_suction_target]
    ...
    def _step(self, action):
        ...
        pyload_pos_limit = ((target_cur[0]<-2.5) or (target_cur[1]<-2.5) or (target_cur[2]<0.0) or (target_cur[0]>2.5) or (target_cur[1]>2.5) or (target_cur[2]>1.5))
       # reach_destination = (target_cur == np.array([0.0,0.0,0.0])).all()
        above_ground = (pos_cur[2]>0.5)

        if ((drone_pos_limit) or (pyload_pos_limit) or(self.step_counter>1200)):
            self._episode_ended = True
            
            if (self.step_counter>1200):
                return ts.termination(self._state, reward = -10.0)
            else:
                return ts.termination(self._state, reward = -15.0)
        else:
            if above_ground:
                ground_reward = 0.25
            else:
            ...
            pos_dev = target_cur - self.suction_cup.get_suction_position()
            suction_reward = -np.power(pos_dev,2).sum()
            
            reward = ground_reward+suction_reward+15*detect 
...
#---------------------------------------------------------------------------------------------------------------
#parameters:
env_name = join(dirname(abspath(__file__)), 'cooperative_transportation_uav1.ttt') # @param {type:"string"}
