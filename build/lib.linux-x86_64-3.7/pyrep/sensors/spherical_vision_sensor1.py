...
class SphericalVisionSensor(Object):
    ...
    def __init__(self, name, suffix):
        ...
        self._sensor_depth = VisionSensor('%s_sensorDepth' % (self.get_name())+suffix)
        self._sensor_rgb = VisionSensor('%s_sensorRGB' % (self.get_name())+suffix)

        # directed sub-sensors
        names = ['front', 'top', 'back', 'bottom', 'left', 'right']
        self._six_sensors = [VisionSensor('%s_%s' % (self.get_name(), n)+suffix) for n in names]
