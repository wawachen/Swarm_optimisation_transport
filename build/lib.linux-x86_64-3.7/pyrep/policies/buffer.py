...
import random
from pyrep.policies.utilities1 import transpose_list
import numpy as np


class ReplayBuffer:
    def __init__(self,agent_size, size, n_steps = 5, discount_rate=0.99):
        self.size = size
        self.num = agent_size
    ...
    def push(self,transition):
        ...
        if len(self.n_step_deque)==self.n_steps:             # once the deque has n steps we can start accumilating rewards
            ...
            summed_reward = np.zeros(self.num)                                    # initialise
            for i,n_transition in enumerate(self.n_step_deque):            # for each experience
                obs, action, reward, next_obs, done = n_transition  
                summed_reward += np.array(reward)*self.discount_rate**(i+1)          # accumulate rewards
                if np.any(done):                                           # stop if done
                    break

            summed_reward = list(summed_reward)
