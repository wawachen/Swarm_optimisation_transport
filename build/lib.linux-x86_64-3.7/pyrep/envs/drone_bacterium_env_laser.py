    ...
    def step(self):
        ...
        for i in range(self.num_a):
            if self.agent_states[i] == 0:
                max_concen[i] = self.agents[i].agent.get_concentration(self.payload)
            else:
                max_concen[i] = 0 
        ...
        for i,agent in enumerate(self.agents):
            ...
            if agent.agent.get_drone_position()[2]<1.3:
                ...
                if self.first_counter[i] == 300:
                    self.agents[i].previous_concentration = self.agents[i].agent.get_concentration(self.payload)
                ...
                if activate == 1:
                    if agent.agent.get_concentration(self.payload) > 0:
                        # q_o = 1.2-agent.agent.get_concentration()
                        q_o = agent.agent.get_concentration(self.payload)
                        vel_obs = np.zeros(3)
                        obstacle_avoidance_velocity = agent.obstacle_avoidance1()
                        vel_obs[0] = obstacle_avoidance_velocity[0]/(q_o*100)
                        vel_obs[1] = obstacle_avoidance_velocity[1]/(q_o*100)
                    ...
                    if self.args.scenario_name=="circle":
                        al = 0.9
                    if self.args.scenario_name=="peanut":
                        al = 0.35
                    if self.args.scenario_name=="square":
                        al = 0.7
                    
                    if agent.agent.get_concentration(self.payload) > (flock_concen*al):
                        
                        if agent.is_neighbour()>0:
                            # print("wawa")
                            flock_vel = agent.flock_controller()
                            bac_vel = agent.bacterium_controller_laser(self.payload)
                        ...
                        if self.args.scenario_name=="square":
                            al1 = 0.5
                        if agent.agent.get_concentration(self.payload) > (flock_concen*al1):
                            bac_vel = agent.bacterium_controller_laser(self.payload) 
                            flock_vel = agent.flock_controller()  
