...
from pyrep.policies.ddpg import DDPGAgent
import torch
...
class MADDPG:
    def __init__(self, p):
        ...
        self.maddpg_agent = [DDPGAgent(p['action_size'], p['action_type'], p['state_size'], p['hidden_in_size'], p['hidden_out_size'], p['num_atoms'], p['lr_actor'], p['lr_critic'],p['l2_decay'],p['noise_type'],p['OU_mu'],p['OU_theta'],p['OU_sigma']) for _ in range(6)]
    ...
    def act(self, obs_all_agents, noise_scale=np.zeros(2)):
        ...
        actions = [agent.act(obs, noise_scale[agent_num]) for agent_num, agent, obs in zip([0,1,2,3,4,5], self.maddpg_agent, obs_all_agents)]
        return actions
    ...
    def update(self, samples, agent_number, logger):
        ...
        #print(map(transpose_to_tensor, samples))
